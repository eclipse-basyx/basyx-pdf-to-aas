...
def main():
    ...
    with open('temp/eclass-properties.json', 'w') as file:
        file.write(json.dumps(property_definitions, indent=2, default=dictionary_serializer))
    with open('temp/eclass-classes.json', 'w') as file:
        file.write(json.dumps(dictionary.classes, indent=2, default=dictionary_serializer))
    ...
    for idx, property_definition in enumerate(property_definitions):
        if idx > 5: break # filter for first 5 properties
        if 'upplier' in property_definition.name['en']: # filter supplier properties, as they are not inside datasheets
            logger.info(f"Skipping {property_definition.id}: {property_definition.name['en']}")
            continue

...
class Camelot(Preprocessor):
    ...
    def __init__(
        self,
        output_format: Literal['html', 'markdown', 'json,', 'csv'] = 'html', # more options are possible, c.f. pandas DataFrame
        accuracy_level: float = 0.8,
    ) -> None:
        self.output_format = output_format
        self.accuracy_limit = accuracy_level
    
    def convert(self, filepath: str) -> list[str] | str | None:
        """
        Converts the content of a PDF file into a list of strings, where each
        string represents a table from the pdf in the desired `output_format` 
        ...
        for table in tables:
            ...
            match self.output_format:
                case 'html': result.append(table.df.to_html())
                case 'json': result.append(table.df.to_json())
                case 'csv': result.append(table.df.to_csv())
                case 'markdown': result.append(table.df.to_markdown())
        return result
